...
public class UserRegistrationManager {
    static String FAIL_PASSWORDS_DONT_MATCH = "Passwords do not match";
    static String FAIL_INVALID_PASSWORD = "Passwords must only be mixed case letters, numbers, and be 8-32 characters";
    static String FAIL_INVALID_USERNAME = "Username must be only letters and be no more than 12 characters";
    static String PASSWORD_REGEX = "^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)[a-zA-Z\\d]{8,32}$";
    static String USERNAME_REGEX = "^[a-zA-Z0-9]{1,12}$";
    @Autowired
    ...
    public RegisterResponse createNewUser(RegisterRequest request) {
        ...
        if(request.getUser() == null || !validateUsername(request)){
            log.warn(FAIL_INVALID_USERNAME);
            response.setMessage(FAIL_INVALID_USERNAME);
        }
        else if(request.getPass1() == null || request.getPass2() == null || !validatePasswordValid(request)){
    ...
    void registerUserInDB(RegisterRequest request){
